...
db_config = {
    ...
    'database': 'kanban_db'  # Reemplaza con el nombre de tu base de datos
}
...
def submit():
    ...
    try:
        ...
        return f"Error al conectar a la base de datos: {err}", 500
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            query = """
                ...
                VALUES (%s, %s, %s)
            """
            cursor.execute(query, (nombre_completo, correo, hashed_password.decode('utf-8')))
            conn.commit()

            return redirect(url_for('principal'))
        except mysql.connector.Error as err:
            return f"Error al registrar usuario: {err}", 500
        finally:
            if 'cursor' in locals():
                cursor.close()
            if 'conn' in locals():
...
@app.route('/fondos_pantalla')
def fondos_pantalla():
    return render_template("fondos_pantalla.html")
